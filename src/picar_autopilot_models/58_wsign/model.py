...
MIN_FILL_RATIO = 0.58         # backed off from 0.65 — still kills squares (0.637)
MIN_ASPECT_NEAR_SQUARE = 0.80 # backed off from 0.85 — allows mild bbox tilt
BLUE_HSV_LOWER = np.array([85, 80, 40])   # tightened from [85,50,20] — sat 50→80, val 20→40
...
SIGN_MIN_DIAMETER_PX   = 15   # ignore detections smaller than this (kills tiny noise blobs)
SIGN_MAX_DIAMETER_PX   = 35   # ignore detections bigger than this (kills huge blue blobs)
# ───────────────────────────────────────────────────────────────────────────
...
# Idle mode: run detector every IDLE frames (cheap, sign detector ~5-15ms).
# Active mode: run detector every ACTIVE frames (responsive when sign visible).
# Transition: TRANSITION consecutive detection-checks of same outcome flips mode.
#   - IDLE → ACTIVE on TRANSITION consecutive positives
#   - ACTIVE → IDLE on TRANSITION consecutive negatives
SIGN_DETECTION_INTERVAL_IDLE   = 10  # every N frames when no sign nearby
SIGN_DETECTION_INTERVAL_ACTIVE = 1   # every N frames when sign nearby
SIGN_CADENCE_TRANSITION        = 5   # consecutive detection-checks to flip mode
# ───────────────────────────────────────────────────────────────────────────

# Per-direction grace: how long to hold the override after the sign disappears.
# Left turns are protected (turn into your own lane), so we can afford to hold
# the override longer to ensure the turn completes before the model resumes.
# Right turns cross opposing flow — release sooner so the car can adjust quickly.
SIGN_GRACE_FRAMES_LEFT  = 10   # hold left override longer
SIGN_GRACE_FRAMES_RIGHT = 5

# ─── Override persistence (noise gate) ────────────────────────────────────
# Override won't arm until N consecutive detection-checks all see an imminent
# sign of the SAME direction. Filters out single-frame false positives so the
# car doesn't commit to a turn on isolated noise blobs.
#   1 = old behaviour (any single imminent detection arms override)
#   2 = single-frame noise filtered (recommended starting point)
#   3 = needs 3-in-a-row, slower to react but very robust
SIGN_OVERRIDE_PERSISTENCE = 3
# ───────────────────────────────────────────────────────────────────────────
...
def _detect_arrow_direction(roi):
    ...
    # Moderate white-interior floor — kills near-empty blobs (bins, chairs)
    # but keeps small real signs (arrow is ~30% of inner circle = ~14 px at
    # a 15px sign, ~40+ at 25px sign).  Was max(12, 10%) which was too high.
    circle_area = np.pi * (min(bw, bh) * 0.45) ** 2
    if white_px < max(6, 0.05 * circle_area):
        return 'unknown'

    centroid_vote = 0
    ys, xs = np.where(white_in_circle > 0)
    offset_ratio = (np.mean(xs) - bw / 2.0) / (bw / 2.0)
    # ±0.10 — moderate, slightly tighter than original 0.08
    if offset_ratio < -0.10: centroid_vote = -1
    elif offset_ratio > 0.10: centroid_vote = 1
    ...
    if le + re > 0:
        r = le / (re + 1e-6)
        # 1.30 / 0.77 — moderate, slightly tighter than original 1.25/0.75.
        # Was 1.40/0.71 which was too tight for small signs (noisy Sobel).
        if r > 1.30: sobel_vote = -1
        elif r < 0.77: sobel_vote = 1

    # Single-vote agreement is back. The actual noise filter now lives in
    # SIGN_OVERRIDE_PERSISTENCE = 3 outside this function (3 consecutive
    # same-direction imminent checks). Unanimous voting was missing real
    # signs at small diameters where Sobel is unreliable.
...
def detect_signs(image):
    ...
    # Loose mask restored (with tightened bounds: S 50→80, V 20→40) so real
    # signs in less-than-perfect lighting can still be detected. Strict mask
    # still helps fill the white-arrow center.
    ...
    for cnt in contours:
        ...
        diam = max(bw, bh)
        if diam < SIGN_MIN_DIAMETER_PX: continue   # too small — likely noise
        if diam > SIGN_MAX_DIAMETER_PX: continue   # too big — likely large blue object
        if diam > max_dim: continue                # extra ratio-based sanity bound

        cnt_mask = np.zeros((h, w), dtype=np.uint8)
        cv2.drawContours(cnt_mask, [cnt], -1, 255, -1)
        mv = cv2.mean(hsv[:, :, 2], mask=cnt_mask)[0]
        ms = cv2.mean(hsv[:, :, 1], mask=cnt_mask)[0]
        if mv < 75 or ms < 100: continue   # backed off from 110 — let real signs through

        (_, _), radius = cv2.minEnclosingCircle(cnt)
        fill = area / (np.pi * radius * radius + 1e-6)
        # No more "highly saturated → relax fill" special-case: after raising
        # the saturation floor to 110, ms is always ≥ 110 here, so the lenient
        # 0.25 branch would always fire and effectively disable the fill check.
        ...
        # Reject candidates with no recognizable arrow — real signs always have
        # a white left/right arrow inside the blue circle. Big false-positive killer.
        if arrow == 'unknown':
            continue
...
class Model:
    def __init__(self):
        ...
        # Adaptive cadence state — start in IDLE (cheap), promote to ACTIVE on
        # SIGN_CADENCE_TRANSITION consecutive positive checks; demote on same
        # number of consecutive negative checks.
        self.current_detection_interval = SIGN_DETECTION_INTERVAL_IDLE
        self.frames_since_last_check    = 0   # increments every frame
        self.consec_positive_checks     = 0   # only updates on detection-frames
        self.consec_negative_checks     = 0

        # Override persistence (noise gate) — track consecutive checks where the
        # SAME direction was seen as imminent. Resets on miss or direction flip.
        self.consec_imminent_checks = 0
        self.last_imminent_arrow    = None  # 'left' or 'right' or None
        ...
        print(f"[INFO] SIGNS enabled — adaptive cadence "
              f"idle={SIGN_DETECTION_INTERVAL_IDLE}f / active={SIGN_DETECTION_INTERVAL_ACTIVE}f "
              f"(switch after {SIGN_CADENCE_TRANSITION} consec checks), "
              f"imminent_px=L{SIGN_IMMINENT_PX_LEFT}/R{SIGN_IMMINENT_PX_RIGHT}, "
              f"override_persistence={SIGN_OVERRIDE_PERSISTENCE}, "
              f"stabilize≥{SIGN_STABILIZE_MIN_PX}px→{SIGN_STABILIZE_ANGLE}°"
              f"{f'±{SIGN_STABILIZE_BAND}' if SIGN_STABILIZE_BAND > 0 else ''}, "
              f"grace_frames=L{SIGN_GRACE_FRAMES_LEFT}/R{SIGN_GRACE_FRAMES_RIGHT}, "
    ...
    def predict(self, image):
        ...
        # ─── 2. Sign detection (adaptive cadence) ───
        # Run detector every `current_detection_interval` frames. After
        # SIGN_CADENCE_TRANSITION consecutive positive (or negative) detection-
        # checks, flip mode between IDLE (cheap) and ACTIVE (responsive).
        self.frames_since_last_check += 1
        ran_detection = False
        if self.frames_since_last_check >= self.current_detection_interval:
            self.last_sign_detections = self._run_signs(image)
            self.frames_since_last_check = 0
            ran_detection = True

            if len(self.last_sign_detections) > 0:
                self.consec_positive_checks += 1
                self.consec_negative_checks = 0
                if (self.current_detection_interval == SIGN_DETECTION_INTERVAL_IDLE
                        and self.consec_positive_checks >= SIGN_CADENCE_TRANSITION):
                    self.current_detection_interval = SIGN_DETECTION_INTERVAL_ACTIVE
            else:
                self.consec_negative_checks += 1
                self.consec_positive_checks = 0
                if (self.current_detection_interval == SIGN_DETECTION_INTERVAL_ACTIVE
                        and self.consec_negative_checks >= SIGN_CADENCE_TRANSITION):
                    self.current_detection_interval = SIGN_DETECTION_INTERVAL_IDLE
                    # Stale detection from idle-mode last check would mislead the
                    # state machine — clear so override grace counter starts ticking.
                    self.last_sign_detections = []
        ...
        imminent_arrow_this_frame = None   # 'left'/'right' if any imminent seen
        for sign in self.last_sign_detections:
            ...
            if dist == 'imminent' and arr in ('left', 'right'):
                imminent_arrow_this_frame = arr   # remember first imminent direction
        ...
        # Update override-persistence counter ONLY on detection-frames so cadence
        # mode doesn't double-count or starve the gate.
        if ran_detection:
            if imminent_arrow_this_frame is not None:
                if imminent_arrow_this_frame == self.last_imminent_arrow:
                    self.consec_imminent_checks += 1
                else:
                    # Direction flipped (or first detection) — reset to 1
                    self.consec_imminent_checks = 1
                    self.last_imminent_arrow = imminent_arrow_this_frame
            else:
                self.consec_imminent_checks = 0
                self.last_imminent_arrow = None

        # Arm override only after SIGN_OVERRIDE_PERSISTENCE consecutive imminent
        # checks of the same direction. Filters single-frame false positives.
        if (self.active_override_angle is None
                and self.consec_imminent_checks >= SIGN_OVERRIDE_PERSISTENCE
                and self.last_imminent_arrow in ('left', 'right')):
            if self.last_imminent_arrow == 'left':
        ...
        # Per-direction grace: hold left longer than right.
        if self.active_override_angle is not None:
            grace = (SIGN_GRACE_FRAMES_LEFT
                     if self.active_override_angle == SIGN_LEFT_ANGLE
                     else SIGN_GRACE_FRAMES_RIGHT)
            if self.frames_since_seen_sign < grace:
        ...
        mode = 'ACT' if self.current_detection_interval == SIGN_DETECTION_INTERVAL_ACTIVE else 'IDL'
        det_str = '*' if ran_detection else ' '
        print(f'Raw angle: {pred_angle_raw:.3f} → {angle:.0f}° | '
              f'Raw speed: {pred_speed_raw:.3f} → {speed} | '
              f'sign[{mode}{det_str}]={n_signs}{ovr_str}{sign_str}')
