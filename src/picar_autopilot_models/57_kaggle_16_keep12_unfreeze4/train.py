...
CONFIG = {
    "EXPERIMENT_NAME": "57_kaggle_16_keep12_unfreeze4",
    "DESCRIPTION": "Parameters and architecture from 16, but using cleaned and corrected dataset, adding more data from youssef. Balance then split, no early stopping, 250 epochs, crop top 120, cut block 12, unfreeze 4",
...
# --- Data Augmentation ---
    ...
    "EPOCHS_FINETUNE": 245,          # Train unfrozen base with low LR
...
def prepare_data_pipelines():
    ...
    # Calculate dynamic joint-distribution weights
    print("[INFO] Calculating dynamic joint-distribution weights...")
    joint_counts = df.groupby(['angle', 'speed']).size()
    total_samples = len(df)
    num_bins = len(joint_counts)
    
    def get_weight(row):
        count = joint_counts.get((row['angle'], row['speed']), 1)
    ...
    if CONFIG.get("BALANCE_VALIDATION", False):
        ...
        train_df = train_df.sample(n=len(train_df), replace=True, weights='sample_weight', random_state=42)
