...
CONFIG = {
    "EXPERIMENT_NAME": "default_baseline16",  # placeholder — every Colab cell overrides this
    "DESCRIPTION": "Dean exp 16 architecture: CUT_AT_BLOCK=10, FREEZE_UP_TO_BLOCK=6, NUM_ATTN_BLOCKS=2",
    ...
    # Note: TEST_IMG_DIR + SUBMISSION_TEMPLATE were Kaggle-only — removed (live testing path)
    "BAD_IMG_CSV": os.path.join("data", "bad_images.csv"),
    "CORRECTIONS_CSV": os.path.join("data", "corrections.csv"),
    "USE_CLEAN_DATA": True,
    "DRIVE_SAVE_PATH": None,  # set by Colab cell, e.g. /content/drive/MyDrive/picar_models/<EXP>_best.h5
...
# ==============================================================================
# CONFIG OVERRIDES from environment variable CONFIG_OVERRIDES (JSON string)
# Used by Colab ablation cells to vary one config without forking the script
# ==============================================================================
import json as _json_mod
_overrides_str = os.environ.get("CONFIG_OVERRIDES", "{}")
try:
    _overrides_dict = _json_mod.loads(_overrides_str)
    if _overrides_dict:
        print(f"[INFO] Applying {len(_overrides_dict)} CONFIG overrides from env: {list(_overrides_dict.keys())}")
        CONFIG.update(_overrides_dict)
except Exception as _e:
    print(f"[WARN] Could not parse CONFIG_OVERRIDES env var: {_e}")

# Derived Input Shape based on (possibly overridden) config
CONFIG["INPUT_SHAPE"] = (
...
def prepare_data_pipelines():
    ...
    # Apply corrections.csv (overwrites angle/speed labels per Dean's pattern)
    corr_path = CONFIG.get("CORRECTIONS_CSV", "")
    if CONFIG.get("USE_CLEAN_DATA", True) and os.path.exists(corr_path):
        # corrections.csv has VARIABLE column count: header is 3 cols (file,variable,value)
        # but data rows can be 3 cols OR 4 cols (with extra 'context' field).
        # Solution: skip header row, force 4 columns, fill missing with NaN.
        try:
            corr_df = pd.read_csv(
                corr_path,
                header=None,                                       # ignore the 3-col header
                skiprows=1,                                         # skip header row
                names=['file', 'variable', 'value', 'context'],    # force 4 cols
                engine='python',
                on_bad_lines='skip'
            )
        except TypeError:
            corr_df = pd.read_csv(
                corr_path, header=None, skiprows=1,
                names=['file', 'variable', 'value', 'context'],
                engine='python', error_bad_lines=False
            )
        corr_df = corr_df.dropna(subset=['file', 'variable', 'value'])
        if len(corr_df) > 0:
            corr_df['image_id'] = corr_df['file'].astype(float).astype(int)
            corr_df['value'] = pd.to_numeric(corr_df['value'], errors='coerce')
            corr_df = corr_df.dropna(subset=['value'])

            # Pivot long→wide: index=image_id, columns={angle,speed}
            # aggfunc='first' to handle any duplicates (keep first correction)
            corr_wide = corr_df.pivot_table(index='image_id', columns='variable',
                                             values='value', aggfunc='first')
            # Dedupe (already unique from pivot, but safety)
            corr_wide = corr_wide[~corr_wide.index.duplicated(keep='first')]

            # Set df index to image_id (int) — handles duplicates via Series.update broadcasting
            df['_idx'] = df['image_id'].astype(float).astype(int)
            df = df.set_index('_idx')

            n_a = n_s = 0
            if 'angle' in corr_wide.columns:
                angle_series = corr_wide['angle'].dropna()
                n_a = len(angle_series.index.intersection(df.index))
                # Use Series.update — properly handles index alignment + duplicates
                new_angle = df['angle'].copy()
                new_angle.update(angle_series)
                df['angle'] = new_angle
            if 'speed' in corr_wide.columns:
                speed_series = corr_wide['speed'].dropna()
                n_s = len(speed_series.index.intersection(df.index))
                new_speed = df['speed'].copy()
                new_speed.update(speed_series)
                df['speed'] = new_speed

            df = df.reset_index(drop=True)
            print(f"[INFO] Applied {n_a} angle + {n_s} speed corrections from {corr_path}")
        else:
            print(f"[WARN] {corr_path} read but no valid corrections found")
    else:
        print(f"[INFO] corrections.csv not applied (USE_CLEAN_DATA={CONFIG.get('USE_CLEAN_DATA',True)}, exists={os.path.exists(corr_path)})")

    # Drop flagged images (graceful fallback if file missing)
    bad_csv = CONFIG["BAD_IMG_CSV"]
    if os.path.exists(bad_csv):
        ...
        print(f"[INFO] Dropped {initial_count - len(df)} bad images ({len(bad_list)} flagged in {bad_csv})")
    else:
        print(f"[WARN] {bad_csv} NOT FOUND — no bad-image filtering applied!")
...
def build_initial_model():
    ...
    # Use BCE for speed when SPEED_AS_CLASSIFICATION=True (consistent across warmup + finetune)
    speed_loss = "binary_crossentropy" if CONFIG.get("SPEED_AS_CLASSIFICATION", False) else CONFIG["LOSS_FUNCTION"]
    speed_metrics = ['accuracy', 'mae'] if CONFIG.get("SPEED_AS_CLASSIFICATION", False) else ['mae']
    model.compile(optimizer=opt,
                  loss={'angle_output': CONFIG["LOSS_FUNCTION"], 'speed_output': speed_loss},
                  metrics={'angle_output': ['mae'], 'speed_output': speed_metrics})
...
def main():
    ...
    if CONFIG["EPOCHS_FINETUNE"] > 0:
        ...
        # ─── BN LOCK toggle (experiment 52_bn_locked) ───
        if CONFIG.get("LOCK_BATCHNORM", False):
            n_bn_locked = 0
            for layer in base_model.layers:
                if isinstance(layer, layers.BatchNormalization):
                    layer.trainable = False
                    n_bn_locked += 1
            print(f"[INFO] LOCK_BATCHNORM: locked {n_bn_locked} BatchNorm layers")

        # Loss config (handle Huber + BCE for experiment 46)
        speed_loss = "binary_crossentropy" if CONFIG.get("SPEED_AS_CLASSIFICATION", False) else CONFIG["LOSS_FUNCTION"]
        speed_metrics = ['accuracy', 'mae'] if CONFIG.get("SPEED_AS_CLASSIFICATION", False) else ['mae']

        # ─── PROGRESSIVE UNFREEZING (experiment 48_progressive) ───
        if CONFIG.get("USE_PROGRESSIVE_UNFREEZING", False):
            print("[INFO] PROGRESSIVE UNFREEZING enabled")
            for layer in base_model.layers:
                if not isinstance(layer, layers.BatchNormalization):
                    layer.trainable = False
            cut_block = CONFIG.get("CUT_AT_BLOCK", 16) or 16
            freeze_to = CONFIG["FREEZE_UP_TO_BLOCK"]
            n_steps = max(1, cut_block - freeze_to)   # e.g. 10-6 = 4 steps for blocks 7,8,9,10
            total_finetune = CONFIG["EPOCHS_FINETUNE"]
            # Allocate epochs: half on progressive ramp-up, half on stable all-unfrozen
            ramp_epochs = total_finetune // 2          # 97 epochs for ramp
            stable_epochs = total_finetune - ramp_epochs  # 98 epochs all-unfrozen
            epochs_per_step = max(1, ramp_epochs // n_steps)
            lr_decay = CONFIG.get("PROGRESSIVE_LR_DECAY", 0.85)
            current_lr = CONFIG["LEARNING_RATE_FINETUNE"]
            current_epoch = CONFIG["EPOCHS_WARMUP"]
            total_budget = CONFIG["EPOCHS_WARMUP"] + CONFIG["EPOCHS_FINETUNE"]
            print(f"[INFO] PROGRESSIVE: {n_steps} steps × {epochs_per_step} epochs = {n_steps*epochs_per_step} ramp epochs, "
                  f"then {stable_epochs} stable epochs (total {total_finetune} finetune)")

            # Phase 2a: progressive ramp — unfreeze one block at a time
            for target_block in range(cut_block, freeze_to, -1):
                for layer in base_model.layers:
                    if f"block_{target_block}_" in layer.name and not isinstance(layer, layers.BatchNormalization):
                        layer.trainable = True
                opt = optimizers.Adam(learning_rate=current_lr)
                model.compile(optimizer=opt,
                              loss={'angle_output': CONFIG["LOSS_FUNCTION"], 'speed_output': speed_loss},
                              metrics={'angle_output': ['mae'], 'speed_output': speed_metrics})
                next_epoch = min(current_epoch + epochs_per_step, total_budget)
                print(f"[INFO] PROGRESSIVE ramp: unfreezing block {target_block}, "
                      f"epochs {current_epoch}-{next_epoch}, LR={current_lr:.2e}")
                h = model.fit(train_ds, validation_data=val_ds,
                              epochs=next_epoch, initial_epoch=current_epoch,
                              callbacks=callbacks)
                combined_val_loss.extend(h.history.get('val_loss', []))
                current_epoch = next_epoch
                current_lr *= lr_decay
                if current_epoch >= total_budget:
                    break

            # Phase 2b: stable training with all blocks unfrozen, remaining budget
            if current_epoch < total_budget:
                opt = optimizers.Adam(learning_rate=current_lr)
                model.compile(optimizer=opt,
                              loss={'angle_output': CONFIG["LOSS_FUNCTION"], 'speed_output': speed_loss},
                              metrics={'angle_output': ['mae'], 'speed_output': speed_metrics})
                print(f"[INFO] PROGRESSIVE stable: all blocks unfrozen, "
                      f"epochs {current_epoch}-{total_budget}, LR={current_lr:.2e}")
                h = model.fit(train_ds, validation_data=val_ds,
                              epochs=total_budget, initial_epoch=current_epoch,
                              callbacks=callbacks)
                combined_val_loss.extend(h.history.get('val_loss', []))

            history_2 = type('H', (), {'history': {'val_loss': combined_val_loss}})()
        else:
            # Standard (Dean's): all-at-once unfreeze
    ...
    # Kaggle submission generation REMOVED — we're doing live testing on Pi, not Kaggle
    # (Original block read sample_submission.csv + ran inference on test_data + saved submission.csv)
    if wandb.run is not None:
        wandb.finish()
    print(f"[SUCCESS] Best model saved to {os.path.join(exp_dir, 'best_model.h5')}")
    print(f"[INFO] Best epoch: {best_epoch_human}, val_loss: {best_val_loss:.5f}")
    print("======================================================================\n")
